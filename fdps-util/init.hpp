...
// [L] = 1[Rearth] = 6.37e8[cm]
// [M] = 1[Msun] = 1.989e30[kg]
// [M] = 1[Mearth] = 5.97e27[g]
...
void MakeKeplerDisk(PS::F64 &mass_planet_glb,
                    ...
                    /*const*/ double &dens,      // [g/cm^2]
                    const double mass_earth = 1.0, //[m_sun]->[M_earth]
                    ...
                    const int seed = 0)
{
    ...
    mass_planet_glb = 4.0 * 0.0123; //[Mearth]
    ...
    dens = mass_planet_glb * mass_earth_gram / (2.0 * PI / (2.0 + power) * (pow(a_out, 2.0 + power) - pow(a_in, 2.0 + power))* REARTH * REARTH );
    ...
    const double h = pow(2.0 * m_planet / (3.0 * mass_earth), 1.0 / 3.0);
    ...
    for (long long int i = 0; i < n_loc; i++)
    {
        ...
        double u = MY_LIB::solve_keplereq(l, ecc); // eccentric anomayl
        MY_LIB::OrbParam2PosVel(pos_dummy, pos[i], vel_dummy, vel[i],
                                mass_earth, m_planet, ax, ecc,
                                inc, OMG, omg, u);
        e_ave += ecc * ecc;
    }
...
void SetParticleKeplerDisk(Tpsys &psys,
                           ...
                           /*const*/ PS::F64 &dens,      // [g/cm^2]
                           const PS::F64 mass_earth = 1.0, //[m_earth]
                           const double a_ice = 0.0,
                           const double f_ice = 1.0,
                           const double power = -1.5,
                           const PS::S32 seed = 0)
{
    ...
    MakeKeplerDisk(mass_planet_glb, mass, pos, vel, n_glb, n_loc,
                   ax_in, ax_out, ecc_rms, inc_rms, dens, mass_earth, a_ice, f_ice, power, seed);
